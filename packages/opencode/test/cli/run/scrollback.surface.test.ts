...
test("turn summary starts at the left edge", async () => {
  ...
  try {
    await out.scrollback.writeTurnSummary({ agent: "Build", model: "Little Frank", variant: "high", duration: "2.2s" })

    const commits = claim(out.renderer)
    try {
      expect(renderRows(commits.at(-1)!)[0]).toBe("▣ Build · Little Frank · high · 2.2s")
    } finally {
      destroy(commits)
    }
  } finally {
