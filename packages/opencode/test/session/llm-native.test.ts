...
describe("session.llm-native.request", () => {
  ...
  it.effect("fails and cancels a silent native provider stream after 60 seconds", () =>
    Effect.gen(function* () {
      let cancelled = false
      ...
      expect(failure).toEqual(expect.objectContaining({ message: "Native LLM stream timed out after 60 seconds" }))
